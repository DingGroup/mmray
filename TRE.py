...
class TREActor:
    ...
    """
    def __init__(self, system, integrator, platform_name,
                 temperature, topology,
                 dcd_file_name, initial_positions = None):
        ...
        self.dcd_file = omm.app.dcdfile.DCDFile(file_handle, self.topology,
...
class TRE():
    ...
    def _exchange_temperature(self):
        potential_energies = ray.get([actor.get_potential_energy.remote()
                                      for actor in self.actors])
        actor_temperatures = ray.get([actor.get_temperature.remote()
                                      for actor in self.actors])
        kbTs = [unit.BOLTZMANN_CONSTANT_kB*unit.AVOGADRO_CONSTANT_NA*(T*unit.kelvin)
    ...
    def run(self, num_steps, exchange_freq, save_freq):
        if exchange_freq < save_freq and save_freq % exchange_freq != 0:
            raise ValueError("save_freq has to be divisible by exchange_freq when it is larger than exchange_freq")
        if exchange_freq > save_freq and exchange_freq % save_freq != 0:
            raise ValueError("exchange_freq has to be divisible by save_freq when it is larger than save_freq")
    ...
    def load_checkpoints(self, checkpoints):
        ...
        [self.actors[i].load_checkpoint.remote(checkpoints[i]) for i in range(len(checkpoints))]
